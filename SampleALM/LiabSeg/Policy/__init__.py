...
def inv_return_mth(t):
    ...
    asset_id = input_data.segment_map.loc[liab_id, 'asset_id']
    return asset_seg[asset_id].seg_mv_return(t)
...
def model_point_table_ext():
    ...
    # start_id, end_id = input_data.segment_data.loc[liab_id, ['start_id', 'end_id']]
    # point_seg = input_data.model_point_10000.loc[start_id:end_id]
    return input_data.model_point_segment(liab_id).join(product_spec_table, on='spec_id')
...
def liab_cf(t):
    ...
    """
    # asset_id = input_data.segment_map.loc[liab_id, 'asset_id']
    # asset_seg[asset_id].liab_cashflows(t)
    return sum(premiums(t) - claims(t) - expenses(t) - commissions(t))


# ---------------------------------------------------------------------------
# References

disc_rate_ann = ("Pickle", 2344525504944)

mort_table = ("Pickle", 2344555484400)

np = ("Module", "numpy")

pd = ("Module", "pandas")

std_norm_rand = ("Pickle", 2344555015168)

surr_charge_table = ("Pickle", 2344554996576)

product_spec_table = ("Pickle", 2344556818224)

model_point_samples = ("Pickle", 2344556817264)

scen_id = 1

model_point_table = ("Pickle", 2344556817264)

input_data = ("Interface", ("...", "InputData"), "auto")

asset_seg = ("Interface", ("...", "Bonds"), "auto")